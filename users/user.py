...
async def loginBot(message:Message, state:FSMContext):
    xabar = message.text.lower()
    for i in readlogin():
        cnt3.append(i[4].lower()) 
    if  xabar in cnt3:
...
async def KirishBot(message:Message):
    await message.answer("🙋🏻‍♂️Assalomu Alaykim xurmatli foydalanuvchi ushbu bot ❓ Quiz bot xisoblanib kar bir o'zuvchi uchun shaxsiy 🔐 login, parollarini saqlagan xolda kirish-chiqishni nazorat qiladi va 👨🏻‍💻 adminga har bir o'quvchini ko'rsatayotgan 📈 natijalarini chiqarib boradi! 🚀", reply_markup=Buttom_user_no)
...
Foiz: {i[5]} %""", reply_markup=Buttom_user_yes)    
        return
    await message.answer("Siz hali test ishlamagansiz 😊", reply_markup=Buttom_user_yes)  
