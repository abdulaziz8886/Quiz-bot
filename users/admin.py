...
async def fanNomBot(message:Message, state:FSMContext):
    for i in readFanler():
        cnt.append(i[1].lower())
    if message.text.lower() in cnt:
        await message.answer("Bunday fan allaqachon qo'shilgan", reply_markup=Buttom_admin)
        await state.clear()
    else:
...
async def usBot(message:Message):
    await message.answer(f"User darajasiga o'tkazildi\n📝 Login: admin\n🔑 Parol: admin54321", reply_markup=Buttom_user_no)
